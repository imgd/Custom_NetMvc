...
$(function(){
	$('#grid1').datagrid({
		...
		//url:'/Ajax/Product/List.aspx',	
		columns:[[
			{title:'', field:'xx', align:'center', width: 40,
				formatter:function(value,row){ return '<a href="/Ajax/Product/Delete.aspx?id=' + row.ProductID + '" title="删除" class="easyui-linkbutton" plain="true"><img src="/Images/delete.gif" alt="删除" /></a>'; }
			},
	...
	$.ajax({
		url: '/Ajax/Category/GetList.aspx', dataType: "json",
			...
			if(data.length > 0){
				...
				$("#ddlCurrentCategory").combobox({ onSelect: function(){
					$('#grid1').datagrid({ queryParams: {PageIndex: 1}, 
							url: '/Ajax/Product/List.aspx?CategoryId=' + $("#ddlCurrentCategory").combobox("getValue") });
					setPaginationEvent();
				}});
				
				$('#grid1').datagrid({url: '/Ajax/Product/List.aspx?CategoryId=' + data[0].CategoryID });
				setPaginationEvent();
			}
			else{
	...
	$("#ddlUnit").combobox({
		valueField:'text', textField:'text', url: "/Ajax/Product/GetUnitList.aspx"
	});
...
function ShowEditDialog(){
	...
	$.ajax({
		url: "/Ajax/Product/GetById.aspx?id=" + productId,  type: "POST", dataType: "json",
		success: function(json) { 
			...
			ShowEditItemDialog(productId, 'divProductInfo', 620, 450, function(j_dialog){
				...
				if (ValidateForm() ) {
					var j_waitDialog = ShowWaitMessageDialog();
					$("#formCreateProduct").ajaxSubmit({
						url: "/Ajax/Product/Update.aspx",  type: "POST", dataType: "text",
...
function SetQuantityTextboxEvent(){
	$("input.quantityTextbox").change(function(){
		...
		$.ajax({
			...
			url: "/Ajax/Product/ChangeProductQuantity.aspx",
