...
$(function() {
	$('#grid1').datagrid({
		...
		url:'/Ajax/Category/List.aspx',		
		columns:[[
			{title:'', field:'xx', align:'center', width: 40,
				formatter:function(value,row){ return '<a href="/Ajax/Category/Delete.aspx?id=' + row.CategoryID + '" title="删除" class="easyui-linkbutton" plain="true"><img src="/Images/delete.gif" alt="删除" /></a>'; }
			},
...
function ShowNewDialog(){
	...
	ShowEditItemDialog('', 'divAddItem', 500, 200, function InsertCategory(j_dialog){
		...
		$.ajax({
			url: "/Ajax/Category/Insert.aspx",  type: "POST", 
...
function ShowEditDialog(){
	...
	ShowEditItemDialog(categoryId, 'divAddItem', 500, 200, function(j_dialog){
		...
		if (ValidateForm() ) {
			var j_waitDialog = ShowWaitMessageDialog();
			$.ajax({
				url: "/Ajax/Category/Update.aspx?CategoryID=" + categoryId,  type: "POST", 
				data: $("#divAddItem :text").fieldSerialize(),
				complete: function() { HideWaitMessageDialog(j_waitDialog); },
				success: function(responseText) {
					$.messager.alert(g_MsgBoxTitle, "操作成功。", "info", function(){ 
